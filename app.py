...
def generate_ndvi_gif(study_roi, index_name='NDVI', start_date='2023-01-01', end_date='2023-03-01'):
    ...
    try:
        st.info(f"🔄 Generating {index_name} timelapse GIF...")
        gif_url = collection.getVideoThumbURL(vis_params)
        st.image(gif_url, caption=f"{index_name} Timelapse")
        st.success(f"✅ {index_name} GIF generated successfully!")
    except Exception as e:
        st.error(f"❌ Error generating {index_name} GIF: {str(e)}")
...
def analyze_vi_timeseries(study_roi, index_name='NDVI', start_date='2020-01-01', end_date=None):
    ...
    try:
        ...
        st.info(f"🔄 Generating {index_name} time series data for {start_date} to {end_date}...")
        ...
        # Display statistics
        st.write(f"📊 {index_name} Statistics for the region ({start_date} to {end_date}):")
        st.write(f"   Mean {index_name}: {monthly_mean.mean():.3f}")
        st.write(f"   Max {index_name}: {monthly_mean.max():.3f}")
        st.write(f"   Min {index_name}: {monthly_mean.min():.3f}")
        st.write(f"   Seasonal variation: {monthly_mean.max() - monthly_mean.min():.3f}")
        
    except Exception as e:
        st.error(f"❌ Error generating {index_name} time series: {str(e)}")

# 6. STREAMLIT UI
def main():
    st.set_page_config(layout="wide", page_title="Advanced Vegetation Index Analysis")
    
    st.title("🌿 Advanced Vegetation Index Analysis")
    st.markdown("Select administrative boundaries or draw custom area, then generate vegetation index visualizations")
    ...
    with col1:
        boundary_type = st.selectbox(
            "Boundary Type:",
            ['Draw on Map', 'Country', 'Admin1 (State/Province)', 'Admin2 (Municipality)'],
            index=1
        )
    ...
    if boundary_type == 'Country':
        ...
        try:
            ...
            st.error(f"Error displaying country: {str(e)}")
            
    elif boundary_type == 'Admin1 (State/Province)':
        selected_country = st.selectbox("Country:", country_options)
        ...
        try:
            ...
            selected_admin1 = st.selectbox("State/Province:", admin1_options)
            selected_admin2 = None
            
            # Update map to show selected admin1
            ...
            st.error(f"Error displaying admin1: {str(e)}")
            
    elif boundary_type == 'Admin2 (Municipality)':
        selected_country = st.selectbox("Country:", country_options)
        ...
        try:
            ...
            selected_admin2 = st.selectbox("Municipality:", admin2_options)
            
            # Update map to show selected admin2
            admin2_feature = FAO_GAUL_ADMIN2.filter(ee.Filter.eq('ADM2_NAME', selected_admin2)).first()
            admin2_geom = admin2_feature.geometry().getInfo()
            
            m = folium.Map(location=[20, 0], zoom_start=8)
            folium.GeoJson(admin2_geom,
                         style_function=lambda x: {'color': 'red', 'fillOpacity': 0.3}).add_to(m)
            
            st_data = st_folium(m, width=700, height=400)
            
        except Exception as e:
            st.error(f"Error displaying admin2: {str(e)}")
        ...
        if st_data and 'all_drawings' in st_data and st_data['all_drawings']:
            st.session_state.drawn_features = st_data['all_drawings'][0]
    
    # Custom data input
    st.subheader("Enter Your NDVI Data (Optional):")
...
2023-01-15,0.235"""
    ...
    data_input = st.text_area(
        "Paste your NDVI data here (date,value):",
        value=default_data,
        height=150
    )
    ...
    if st.button("Generate Visualizations", type="primary"):
        ...
        if boundary_type == 'Draw on Map':
            if not st.session_state.drawn_features:
                st.error("❌ Please draw a boundary on the map first!")
                return
                
            try:
                study_roi = ee.Geometry(st.session_state.drawn_features['geometry'])
                selected_name = "Custom Drawn Area"
            except Exception as e:
                st.error(f"❌ Error getting drawn features: {str(e)}")
                return
        else:
            if boundary_type == 'Country':
                ...
                if not selected_admin1:
                    st.error("❌ Please select a state/province first!")
                    return
                ...
                if not selected_admin2:
                    st.error("❌ Please select a municipality first!")
                    return
        ...
        # Show study area
        st.subheader(f"📍 Selected Area: {selected_name}")
        st.write(f"📅 Date Range: {start_date_str} to {end_date_str}")
        st.write(f"🌿 Vegetation Index: {selected_vi}")

        # Create a new map to display the selected area
        try:
            m2 = folium.Map(location=[20, 0], zoom_start=6)
            folium.GeoJson(study_roi.getInfo(),
                          style_function=lambda x: {'color': 'green', 'fillOpacity': 0.2}).add_to(m2)
            st_folium(m2, width=700, height=400)
        except Exception as e:
            st.error(f"Error displaying study area: {str(e)}")

        # Generate vegetation index time series analysis
        st.subheader(f"📈 {selected_vi} Time Series Analysis")
        analyze_vi_timeseries(study_roi, selected_vi, start_date_str, end_date_str)

        # Generate satellite GIF
        st.subheader(f"🛰️ Satellite {selected_vi} Timelapse")
        generate_ndvi_gif(study_roi, selected_vi, start_date_str, end_date_str)

        # Process custom data
        if data_input.strip():
            st.subheader("📊 Your NDVI Data Animation")
            analyze_custom_data(data_input)
